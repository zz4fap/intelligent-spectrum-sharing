...
int srslte_rf_recv_wrapper(void *h, void *data, uint32_t nsamples, srslte_timestamp_t *q, size_t channel) {
...
int main(int argc, char **argv) {
  ...
  srslte_rf_set_rx_freq(&rf, (double) prog_args.rf_freq, 0);
  ...
  uint32_t ntrial=0;
  do {
    ret = rf_search_and_decode_mib(&rf, &cell_detect_config, prog_args.force_N_id_2, &cell, &cfo, 0);
    if (ret < 0) {
  ...
  srslte_rf_stop_rx_stream(&rf, 0);
  srslte_rf_flush_buffer(&rf, 0);
  ...
  while ((sf_cnt < prog_args.nof_subframes || prog_args.nof_subframes == -1) && !go_exit) {
    ...
    ret = srslte_ue_sync_get_buffer(&ue_sync, &sf_buffer, 0);
    if (ret < 0) {
    ...
    if (ret == 1) {
      ...
      case MEASURE:
        ...
        if ((nframes%100) == 0 || rx_gain_offset == 0) {
          if (srslte_rf_has_rssi(&rf, 0)) {
            rx_gain_offset = 10*log10(rssi)-srslte_rf_get_rssi(&rf, 0);
          } else {
            rx_gain_offset = srslte_rf_get_rx_gain(&rf, 0);
          }
        }
