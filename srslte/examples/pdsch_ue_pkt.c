...
int srslte_file_recv_wrapper(void *h, void *data, uint32_t nsamples, srslte_timestamp_t *t, size_t channel) {
  DEBUG(" ----  Receive %d samples form file  ---- \n", nsamples);
...
int main(int argc, char **argv) {
  ...
  if (!prog_args.input_file_name) {
    ...
    if (prog_args.rf_gain > 0) {
      srslte_rf_set_rx_gain(&rf, prog_args.rf_gain, 0);
    } else {
      printf("Starting AGC thread...\n");
      if (srslte_rf_start_gain_thread(&rf, false, 0)) {
        fprintf(stderr, "Error opening rf\n");
        exit(-1);
      }
      srslte_rf_set_rx_gain(&rf, 50, 0);
    ...
    srslte_rf_set_rx_freq(&rf, prog_args.rf_freq, 0);
    srslte_rf_rx_wait_lo_locked(&rf, 0);
    ...
    int srate = srslte_sampling_freq_hz(prog_args.file_nof_prb);
    if (srate != -1) {
      if (srate < 10e6) {
      ...
      float srate_rf = srslte_rf_set_rx_srate(&rf, (double) srate, 0);
      if (srate_rf != srate) {
  ...
  if (prog_args.input_file_name) {
    ...
    if (srslte_ue_sync_init_file_new(&ue_sync, prog_args.file_nof_prb,
      prog_args.input_file_name, prog_args.file_offset_time, prog_args.file_offset_freq, cell, srslte_file_recv_wrapper, 0)) {
    ...
    cell.nof_ports = prog_args.file_nof_ports;
    cell.nof_prb = prog_args.file_nof_prb;
    if (srslte_ue_sync_init(&ue_sync, cell, srslte_rf_recv_wrapper, (void*) &rf, 0)) {
      fprintf(stderr, "Error initiating ue_sync\n");
      exit(-1);
    }
#endif
...
#ifndef DISABLE_RF
  if (!prog_args.input_file_name) {
    srslte_rf_start_rx_stream(&rf, 0);
  }
#endif
  ...
  ue_sync.sfind.enable_cfo_corr = ue_sync.correct_cfo;
  //ue_sync.sfind.sss_en = false;
  //ue_sync.sfind.detect_cp = false;
...
//  printf("cfo %d\n", cfo);
  ...
  #if 0
  //-----------sync and pass the 1st sf--------------//
  ret = srslte_ue_sync_get_buffer_new(&ue_sync, &sf_buffer);
  if (ret < 0) {
  ...
  INFO("\nEntering main loop...\n\n", 0);
  /* Main loop */
  while (!go_exit && (sf_cnt < (prog_args.nof_subframes-1) || prog_args.nof_subframes == -1)) {
  //while (!go_exit && (sfn < prog_args.nof_subframes || prog_args.nof_subframes == -1)) {
    //printf("0\n");
    ret = srslte_ue_sync_get_buffer_new(&ue_sync, &sf_buffer, 0);
    if (ret < 0) {
      fprintf(stderr, "Error calling srslte_ue_sync_work()\n");
    }
    //printf("ret %d sf_idx %d\n", ret, ue_sync.sf_idx);
    ...
    if (ret == 1) {
      ...
      switch (state) {
        ...
        case DECODE_PDSCH:
          ...
          //printf("prog_args.rnti %d decode_pdsch %d\n", prog_args.rnti, decode_pdsch);
          if (decode_pdsch) {
            INFO("Attempting DL decode SFN=%d\n", sfn);
            n = srslte_ue_dl_decode(&ue_dl,
                                    //&sf_buffer[prog_args.time_offset],
                                    sf_buffer + ue_sync.frame_len + prog_args.time_offset,
                                    data,
      ...
      //printf("srslte_ue_sync_get_sfidx(&ue_sync) %d\n", srslte_ue_sync_get_sfidx(&ue_sync));
      if (srslte_ue_sync_get_sfidx(&ue_sync) == 9) {
        printf("cid %d ttal %d err %d dtct %d\n", 3*ue_sync.sfind.N_id_1 + ue_sync.sfind.N_id_2, ue_dl.pkts_total, ue_dl.pkt_errors, ue_dl.nof_detected);
        sfn++;
        if (sfn == 1024) {
          sfn = 0;
          printf("\n");
