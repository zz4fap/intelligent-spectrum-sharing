...
int srslte_ue_sync_init_reentry_loop(srslte_ue_sync_t *q) {
  ...
  if(q != NULL) {
    ...
    // After we find a peak and the subframe is aligned to the start of the buffer we need skip shifting the buffer content after the end of the subframe.
    q->is_find_peak_ok = true;
...
static int find_peak_ok(srslte_ue_sync_t *q, size_t channel) {
  ...
  int ret = 0, offset, num_of_samples_to_stay, num_of_samples_to_copy, num_of_remaining_samples;
  ...
  if(q->frame_find_cnt >= q->nof_avg_find_frames || (q->peak_idx-q->frame_len) < 2*q->fft_size) {
    ...
    num_of_remaining_samples = 0;
    if(num_of_samples_to_stay < q->sf_len) {
      num_of_samples_to_copy = num_of_samples_to_stay;
      num_of_remaining_samples = q->sf_len - num_of_samples_to_stay;
    } else {
    ...
    // If all the IQ samples of a subframe are already contained in the current buffer, then there is no need to align it to the buffer begining, we just use the position of the subframe start.
    if(num_of_remaining_samples == 0) {
      ...
      if(srslte_rf_recv_with_time(q->stream, (q->input_buffer[q->subframe_buffer_counter]+num_of_samples_to_copy), num_of_remaining_samples, true, &(q->last_timestamp.full_secs), &(q->last_timestamp.frac_secs), channel) < 0) {
        return SRSLTE_ERROR;
      }
    }
...
static inline int receive_samples_after_sync(srslte_ue_sync_t *q, size_t channel) {

  // Transfer the IQ samples still in the previous buffer to the new buffer.
  ...
  // Get N subframes from the USRP getting more samples and keeping the previous samples, if any
  if(srslte_rf_recv_with_time(q->stream, (q->input_buffer[q->subframe_buffer_counter]+q->frame_len+q->num_of_samples_still_in_buffer), (2*q->sf_len-q->num_of_samples_still_in_buffer), true, &(q->last_timestamp.full_secs), &(q->last_timestamp.frac_secs), channel) < 0) {
    return SRSLTE_ERROR;
  }
...
int srslte_ue_sync_get_subframe_buffer(srslte_ue_sync_t *q, size_t channel) {
  int ret = srslte_ue_sync_zerocopy_new(q, channel);
  if(ret == 1) {
...
int srslte_ue_sync_zerocopy_new(srslte_ue_sync_t *q, size_t channel) {
  ...
  if(q != NULL) {
    ...
    if(q->file_mode) {
      ...
      // Uncomment the lines below to measure the time it takes to read samples and synchronize to a read buffer with IQ samples.
      //struct timespec read_plus_synch_time;
      //clock_gettime(CLOCK_REALTIME, &read_plus_synch_time);

      // Uncomment the lines below to measure the time it takes to read samples from the USRP.
      //struct timespec read_samples_time;
      //clock_gettime(CLOCK_REALTIME, &read_samples_time);

      if(!q->is_find_peak_ok) {
        if(receive_samples(q, channel)) {
          UE_SYNC_ERROR("Error receiving samples.\n",0);
          return SRSLTE_ERROR;
        }
      } else {
        q->is_find_peak_ok = false;
        if(receive_samples_after_sync(q, channel)) {
          UE_SYNC_ERROR("Error receiving samples after sync.\n",0);
          return SRSLTE_ERROR;
        }
      }

      //PHY_PROFILLING_AVG3("Avg. read samples: %f - min: %f - max: %f - max counter %d - diff >= 1ms: %d - total counter: %d - perc: %f\n",helpers_profiling_diff_time(read_samples_time), 1.0, 1000);
...
int srslte_ue_sync_allocate_subframe_buffer(srslte_ue_sync_t *q) {
   for(int i = 0; i < NUMBER_OF_SUBFRAME_BUFFERS; i++) {
      q->input_buffer[i] = srslte_vec_malloc(NUMBER_OF_SUBFRAMES_TO_STORE*q->frame_len*sizeof(cf_t));
      if(!q->input_buffer[i]) {
