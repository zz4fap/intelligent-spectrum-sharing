...
int srslte_ue_cellsearch_init(srslte_ue_cellsearch_t * q, uint32_t max_frames,
                           int (recv_callback)(void*, void*, uint32_t,srslte_timestamp_t*, size_t), void *stream_handler)
{
  ...
  if (q != NULL) {
    ...
    if (srslte_ue_sync_init(&q->ue_sync, cell, recv_callback, stream_handler, 0)) {
...
int srslte_ue_cellsearch_scan(srslte_ue_cellsearch_t * q,
                              ...
                              uint32_t *max_N_id_2, size_t channel)
{
  int ret = 0;
  float max_peak_value = -1.0;
  uint32_t nof_detected_cells = 0;
  for (uint32_t N_id_2=0;N_id_2<3 && ret >= 0;N_id_2++) {
    ret = srslte_ue_cellsearch_scan_N_id_2(q, N_id_2, &found_cells[N_id_2], channel);
    if (ret < 0) {
...
int srslte_ue_cellsearch_scan_N_id_2(srslte_ue_cellsearch_t * q,
                                     uint32_t N_id_2,
                                     srslte_ue_cellsearch_result_t *found_cell, size_t channel)
{
  int ret = SRSLTE_ERROR_INVALID_INPUTS;
  cf_t *sf_buffer = NULL;
  ...
  if (q != NULL)
  {
    ...
    do {
      ...
      ret = srslte_ue_sync_get_buffer(&q->ue_sync, &sf_buffer, channel);
      if (ret < 0) {
