...
// This mutex is used to synchronize the access to the USRP resource when setting frequency, gain, sampling rates, etc.
...
// We use only one mutex as we have TX and RX parameters being modified in different threads.
...
bool rf_uhd_rx_wait_lo_locked(void *h, size_t channel) {
  ...
  bool val = isRxLocked(handler, sensor_name, &value_h, channel);
...
bool rf_uhd_tx_wait_lo_locked(void *h, size_t channel) {
  ...
  bool val = isTxLocked(handler, sensor_name, &value_h, channel);
...
int rf_uhd_open(char *args, void **h) {
  if(h) {
    ...
    *h = handler;
    ...
    if(pthread_mutex_init(&tx_rx_mutex, NULL) != 0) {
...
int rf_uhd_close(void *h) {
  ...
  pthread_mutex_destroy(&tx_rx_mutex);
...
double rf_uhd_set_rx_srate(void *h, double freq, size_t channel) {
  // Lock a mutex prior to using the USRP object.
  pthread_mutex_lock(&tx_rx_mutex);
  rf_uhd_handler_t *handler = (rf_uhd_handler_t*) h;
  uhd_usrp_set_rx_rate(handler->usrp, freq, channel);
  uhd_usrp_get_rx_rate(handler->usrp, channel, &freq);
...
double rf_uhd_set_tx_srate(void *h, double freq, size_t channel) {
  // Lock a mutex prior to using the USRP object.
  pthread_mutex_lock(&tx_rx_mutex);
  rf_uhd_handler_t *handler = (rf_uhd_handler_t*) h;
  uhd_usrp_set_tx_rate(handler->usrp, freq, channel);
  uhd_usrp_get_tx_rate(handler->usrp, channel, &freq);
  handler->channels[channel].tx_rate = freq;
  // Unlock mutex upon using USRP object.
  pthread_mutex_unlock(&tx_rx_mutex);
  return freq;
}
...
double rf_uhd_set_tx_gain(void *h, double gain, size_t channel) {
  ...
  pthread_mutex_lock(&tx_rx_mutex);
  rf_uhd_handler_t *handler = (rf_uhd_handler_t*) h;
  uhd_usrp_set_tx_gain(handler->usrp, gain, channel, "");
  uhd_usrp_get_tx_gain(handler->usrp, channel, "", &gain);
  // Unlock mutex upon using USRP object.
  pthread_mutex_unlock(&tx_rx_mutex);
  return gain;
}
...
double rf_uhd_get_rx_gain(void *h, size_t channel) {
  ...
  pthread_mutex_lock(&tx_rx_mutex);
  rf_uhd_handler_t *handler = (rf_uhd_handler_t*) h;
  uhd_usrp_get_rx_gain(handler->usrp, channel, "", &gain);
  // Unlock mutex upon using USRP object.
  pthread_mutex_unlock(&tx_rx_mutex);
  return gain;
}
                    ...
                    size_t channel)
{
  ...
  if(blocking) {
    ...
    do {
      ...
      //PHY_PROFILLING_AVG3("Avg. USRP read samples: %f - min: %f - max: %f - max counter %d - diff >= 1ms: %d - total counter: %d - perc: %f\n",helpers_profiling_diff_time(read_samples_time), 1.0, 1000);
  ...
  if(blocking) {
    ...
    do {
      ...
      uhd_error error = uhd_tx_streamer_send(handler->channels[channel].tx_stream, buffs_ptr,
                                             tx_samples, &handler->channels[channel].tx_md, 3.0, &txd_samples);
      if(error) {
