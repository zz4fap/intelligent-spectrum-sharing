...
int srslte_sch_init(srslte_sch_t *q) {
  ...
  if (q) {
    ...
    q->filler_bits_error = 0;
    q->nof_cbs_exceeds_softbuffer_size_error = 0;
    q->rate_matching_error = 0;
    q->cb_crc_error = 0;
    q->tb_crc_error = 0;
...
//      printf("cb_segm->tbs %d\n", cb_segm->tbs);
//      printf("q->crc_tb.polynom %d\n", q->crc_tb.polynom);
//      printf("q->crc_tb.order %d\n", q->crc_tb.order);
//      printf("q->crc_tb.crcinit %d\n", q->crc_tb.crcinit);
//      printf("q->crc_tb.crcmask %d\n", q->crc_tb.crcmask);
//      printf("q->crc_tb.table %d %d %d %d\n", q->crc_tb.table[0], q->crc_tb.table[1], q->crc_tb.table[254], q->crc_tb.table[255]);
//      printf("data %d %d %d %d\n", data[0], data[1], data[111], data[112]);
                     ...
                     int16_t *e_bits, uint8_t *data)
{
      ...
      cb_segm      != NULL)
  {
    ...
    if (cb_segm->F) {
      SCH_DEBUG("decode_tb(): Error filler bits are not supported. Use standard TBS\n",0);
      q->filler_bits_error++;
      return SRSLTE_ERROR;
    }

    if (cb_segm->C > softbuffer->max_cb) {
      SCH_DEBUG("decode_tb(): Error number of CB (%d) exceeds soft buffer size (%d CBs)\n", cb_segm->C, softbuffer->max_cb);
      q->nof_cbs_exceeds_softbuffer_size_error++;
      return -1;
    }
    ...
    for (i = 0; i < cb_segm->C && early_stop; i++) {
      ...
      if (srslte_rm_turbo_rx_lut(&e_bits[rp], softbuffer->buffer_f[i], n_e, cblen_idx, rv)) {
        SCH_DEBUG("decode_tb(): Error in rate matching.\n",0);
        q->rate_matching_error++;
        return SRSLTE_ERROR;
      }
    ...
    if (!early_stop) {
      SCH_DEBUG("decode_tb(): CB %d failed. TB is erroneous.\n",i-1);
      q->cb_crc_error++;
      return SRSLTE_ERROR;
    } else {
      INFO("END CB#%d: wp: %d, rp: %d\n", i, wp, rp);
...
//      printf("data %d %d %d %d\n", data[0], data[1], data[111], data[112]);
      ...
      if (!par_rx) {
        SCH_INFO("Warning: Received all-zero transport block\n\n", 0);
      }
      ...
      if (par_rx == par_tx && par_rx) {
        ...
        SCH_DEBUG("decode_tb(): Error in TB parity: par_tx=0x%x, par_rx=0x%x\n", par_tx, par_rx);
        q->tb_crc_error++;
        return SRSLTE_ERROR;
      }
