...
int srslte_sync_init(srslte_sync_t *q, uint32_t frame_size, uint32_t max_offset, uint32_t fft_size, uint32_t radio_id, int initial_subframe_index) {
  ...
  if (q                 != NULL         &&
      frame_size        <= 307200       &&
      fft_size_isvalid(fft_size)        &&
      radio_id          <= MAXIMUM_NUMBER_OF_RADIOS)
  {
    ret = SRSLTE_ERROR;
    ...
    q->sss_alg = SSS_FULL;
    q->node_N_id_1 = radio_id/3;
    q->node_N_id_2 = radio_id%3;
...
srslte_sync_find_ret_t srslte_sync_find(srslte_sync_t *q, cf_t *input, uint32_t find_offset, uint32_t *peak_position)
{
      ...
      fft_size_isvalid(q->fft_size))
  {
    ...
    if(q->find_cfo_i && q->enable_cfo_corr) {
      q->cfo_i = -cfo_i_estimate(q, input_cfo + q->frame_size, find_offset, &peak_pos);
      if(q->cfo_i != 0) {
      ...
      if(peak_pos < 0) {
        SYNC_ERROR("Error calling finding PSS sequence.\n",0);
        return SRSLTE_ERROR;
      }

      // If peak value is greater than threshold and peak position is less than 2*(FFT size + CP size) we try to find a peak starting several samples before.
      if((q->peak_value >= q->threshold) && ((peak_pos + find_offset) < 2*(q->fft_size + SRSLTE_CP_LEN_EXT(q->fft_size)))) {

        DEV_DEBUG("[BEFORE]: peak value: %f - peak position: %d - find offset: %d.\n", q->peak_value, peak_pos, find_offset);

        // try to find peak postion starting half-subframe before.
        peak_pos = srslte_pss_synch_find_pss(&q->pss, &input_cfo[(q->frame_size/2) + find_offset], &q->peak_value);
        if(peak_pos < 0) {
          SYNC_ERROR("Error calling finding PSS sequence.\n",0);
          return SRSLTE_ERROR;
        }

        DEV_DEBUG("[AFTER]: peak value: %f - peak position: %d - find offset: %d.\n", q->peak_value, peak_pos, find_offset);

        // Apply offset to the peak_pos so that we have the real position inside the input vector which is 2 subframes long.
        peak_pos = peak_pos + (q->frame_size/2) + find_offset;
      } else {
    ...
    if(q->peak_value >= q->threshold) {
      ...
      DEV_DEBUG("frame_size: %d - peak_value: %f - find_offset: %d - non-offset peak pos: %d - peak position: %d\n",q->frame_size,q->peak_value, find_offset, (peak_pos - q->frame_size), peak_pos);
      ...
      if(peak_pos >= 2*(q->fft_size + SRSLTE_CP_LEN_EXT(q->fft_size))) {
        float cfo2   = srslte_pss_synch_cfo_compute(&q->pss, input_cfo + peak_pos - q->fft_size);
    ...
    // PHY Filtering. We check if the received PSS/SSS (N_id_1/N_id_2) and SF index were sent to this node.
    if(q->N_id_2 != q->node_N_id_2 || q->N_id_1 != q->node_N_id_1 || q->sf_idx != q->initial_subframe_index) {
