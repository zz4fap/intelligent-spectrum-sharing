...
void srslte_vec_power_spectrum_cf(cf_t *x, float *z, const float normalizationFactor, uint32_t len) {
#ifndef HAVE_VOLK_MULT2_CONJ_FUNCTION
  int i;
  for (i=0;i<len;i++) {
    z[i] = 10.0*log10f(crealf(x[i]*conjf(x[i])));
  }
#else
  volk_32fc_s32f_power_spectrum_32f(z, x, normalizationFactor, len);
#endif
}
