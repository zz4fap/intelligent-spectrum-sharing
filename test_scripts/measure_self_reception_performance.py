...
def inputOptions(argv):
    ...
    try:
        opts, args = getopt.getopt(argv,"hdpswi:b:m:t:r:",["help","debug","profile","single","waittxstats","sendtoid=","bw=","mcs=","txgain=","rxgain=","txslots=","rxslots=","txchannel=","rxchannel=","stats","rfboost=","numpktstotx=","savetofile"])
    ...
    return debug, profiling, single, wait_for_tx_stats, send_to_id, nof_prb, mcs, txgain, rxgain, tx_slots, rx_slots, tx_channel, rx_channel, debugStats, rfboost, num_of_packets_to_tx, saveStatsToFile
...
if __name__ == '__main__':
    ...
    debug, profiling, single, wait_for_tx_stats, send_to_id, nof_prb, mcs, txgain, rxgain, tx_slots, rx_slots, tx_channel, rx_channel, debugStats, rfboost, num_of_packets_to_tx, saveStatsToFile = inputOptions(sys.argv[1:])
    ...
    while(True):
        ...
        if(wait_for_tx_stats == False):
            time.sleep(0.005)
            #time.sleep(0.00001) # Original value
