...
def inputOptions(argv):
    ...
    try:
        opts, args = getopt.getopt(argv,"hdpstc:b:",["help","debug","profile","single","txstats","cid","bw"])
...
def sendRxCtrlToPhy(lc, bandwidth_idx, chan, mcs_idx, rx_gain, num_data_slots, seq_number, module, debug):
...
def sendTxCtrlToPhy(lc, chan, bandwidth_index, mcs_idx, gain_value, slots, sequence_number, send_to_id, module, data, debug):
    ...
    bw_idx      = bandwidth_index   # By default use BW: 5 MHz. Possible values: 0 - 1.4 MHz, 1 - 3 MHz, 2 - 5 MHz, 3 - 10 MHz.
    mcs         = mcs_idx           # It has no meaning for RX. MCS is recognized automatically by receiver. MCS varies from 0 to 28.
    channel     = chan              # By default use channel 0.
    slot        = 0                 # Slot number (not used now, for future use)
            ...
            try:
                ...
                if(internal.receiver.result > 0 and internal.receiver.result == PHY_SUCCESS):
                    if(internal.transaction_index == 66):
                       ...
                       if(len(total_rxd_bytes) > 0 and len(total_rxd_bytes) == getTBSize()):
...
if __name__ == '__main__':
    ...
    mcs = 0
    tx_gain = 50 # Good value for b200: 30 # for x310 use 200
    rx_gain = 50
    num_of_tx_slots = 1
    num_of_rx_slots = 1
    seq_number = 100
    ...
    print("PRB:",nof_prb,"- BW index",bandwidth)
    setTBSize(getTransportBlockSize(bandwidth, mcs));
    ...
    sendRxCtrlToPhy(lc, bandwidth, channel, mcs, rx_gain, num_of_rx_slots, 66, source_module, debug)
